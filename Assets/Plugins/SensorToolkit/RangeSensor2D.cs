...
namespace SensorToolkit
{
    ...
    public class RangeSensor2D : BaseAreaSensor
    {
        ...
        void testSensor()
        {
            ...
            // Use OverlapCircle instead of deprecated OverlapCircleNonAlloc
            var results = Physics2D.OverlapCircle(transform.position, SensorRange, DetectsOnLayers);
            if (results == null)
            {
                clearColliders();
                return;
            ...
            // Create a list or array from the results
            var detectedColliders = Physics2D.OverlapCircleAll(transform.position, SensorRange, DetectsOnLayers);
            
            clearColliders();
            for (int i = 0; i < detectedColliders.Length; i++)
            {
                var newDetection = addCollider(detectedColliders[i]);
