...
public static class ResourcesServices
{
    ...
    public static GameObject LoadCardPrefab(string key, bool isSkill)
    {
        ...
        // Try to extract subfolder from key if it has the right format (e.g. U_ALL_2)
        if (key.Length > 4 && key.Contains("_"))
        {
        ...
        // Build path and attempt to load
        string path = $"Prefabs/{folder}/{subfolder}/{key}";
        Debug.Log(path);
        ...
        if (prefab == null)
        {
            ...
            path = $"Prefabs/{folder}/{key}";
            Debug.Log($"Prefab not found, trying alternate path: {path}");
            ...
            {
                path = key;
                Debug.Log($"Prefab still not found, trying direct path: {path}");
                prefab = Resources.Load<GameObject>(path);
            }
        }
